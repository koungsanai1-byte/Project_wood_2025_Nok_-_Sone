  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`http://localhost:3000/api/deletes_size/${id}`)
          .then(() => {
            Swal.fire({
              icon: 'success',
              title: '✅ ລຶບສຳເລັດ',
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold text-gray-700">ລາຍການ ຂະໜາດໄມ້</h2>
        <Link
          to="/size_create"
            ...
            {size.length === 0 ? (
              ...
              size.map((d, i) => (
                <tr key={i} className="hover:bg-gray-50 border-t">
                  <td className="px-6 py-4">{d.id_size}</td>
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="flex gap-2 justify-center">
                      ...
                      <button
                        onClick={() => handleDelete(d.id_size)}
