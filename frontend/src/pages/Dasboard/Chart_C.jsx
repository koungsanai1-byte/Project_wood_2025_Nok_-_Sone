  ...
  const fetchData = async () => {
    try {
      ...
      const [totalRes, monthlyRes, latestRes, chartRes] = await Promise.all([
        fetch(`${API_BASE_URL}/total-sales`),
        fetch(`${API_BASE_URL}/monthly-sales`),
        fetch(`${API_BASE_URL}/latest-sales`),
        fetch(`${API_BASE_URL}/chart-data-s`)
      ]);
      ...
      setTotalPurchase(totalData.total_sales || 0);
      setMonthlyPurchase(monthlyData.total_sales || 0);
      setLatestPurchase({
        total: latestData.latest_combined_total || 0,
        date_purchase: latestData.date_purchase || null
      });
      setData(chartData || []);
    ...
    <div className="w-full" ref={containerRef}>
      <div className="flex flex-col w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-16">
          ...
          <div className="border rounded-lg p-2">
            <p className="text-xs text-gray-500">ຍອດລ່າສຸດ</p>
            <p className="text-sm font-semibold">
