...
const ResponsiveFinancialChart = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [totalRes, monthlyRes, latestRes, chartRes] = await Promise.all([
        fetch(`${API_BASE_URL}/total-amount`),
        fetch(`${API_BASE_URL}/monthly-amount`),
        fetch(`${API_BASE_URL}/latest-amount`),
        fetch(`${API_BASE_URL}/get_chart`)
      ]);
      ...
      setTotalPurchase(totalData.total_amount || 0);
      setMonthlyPurchase(monthlyData.monthly_amount || 0);
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      // payload[0].value คือค่า amount ของแท่งนั้น
      return (
        <div className="bg-white p-2 border rounded shadow">
          <p className="text-sm ">ຍອດເດືອນນີ້: {formatCurrency(totalPurchase)}</p>
        </div>
      );
    }
    ...
    <div className="w-full" ref={containerRef}>
      <div className="flex flex-col w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-16">
          ...
          <div className="border rounded-lg p-2">
            <p className="text-xs text-gray-500">ຜະລິດລ່າສຸດ</p>
            <p className="text-sm font-semibold">
              {latestPurchase && typeof latestPurchase.amount === 'number'
                ? formatCurrency(latestPurchase.amount)
                : '0'}
            </p>
            <p className="text-xs text-gray-400">
              {latestPurchase && latestPurchase.created_at
                ? formatDate(latestPurchase.created_at)
                : 'N/A'}
            </p>
